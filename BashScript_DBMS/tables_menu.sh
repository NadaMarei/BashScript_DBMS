	...
	then
			...
			then
				...
				then
					...
					recordNumber=$(awk 'BEGIN{FS=";"}{if ($'$fieldNumber'=="'$value'") print recordNumber}' $table 2>> /dev/null)
					# edit files in place | delete the record
					sed -i ''$recordNumber'd' $table 2>> /dev/null
...
function updateRecord {
	...
	then
		...
		table_menu
			...
			recordNumber=$(awk 'BEGIN{FS=";"}{if ($'$fieldNumber' == "'$value'") print recordNumber}' $table 2>> /dev/null)
			echo $recordNumber
			oldValue=$(awk 'BEGIN{FS=";"}{if(NR=='$recordNumber'){for(i=1;i<=NF;i++){if(i=='$fieldNumber') print $i}}}' $table 2>> /dev/null)
			echo $oldValue
			#substitute globally the oldvalue with the newvalue
			sed -i ''$recordNumber's/'$oldValue'/'$newValue'/g' $table 2>> /dev/null
...
# check the existence of the PK --> fieldnumber  dataForThatField
